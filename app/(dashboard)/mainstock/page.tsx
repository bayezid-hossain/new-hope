        ...
        <Table>
          ...
          <TableBody>
            {data?.items.map((row) => {
              ...
              const percentUsed = mainStock > 0 ? (consumed / mainStock) * 100 : 0;
                  ...
                  <TableCell>
                      <div className="flex flex-wrap gap-2">
                        {row.activeCycles.map((c: any) => (
                            <Badge key={c.id} variant="secondary" className="text-xs font-normal border-gray-200">
                                {/* Adjust 'c.name' if your schema uses 'cycleId' or similar */}
                                Cycle <span className="text-muted-foreground ml-1">(Age: {c.age})</span>
                            </Badge>
                        ))}
                        {row.activeCycles.length === 0 && <span className="text-muted-foreground text-xs italic">No active cycles</span>}
                      </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="space-y-1.5">
                        <div className="flex justify-between text-xs">
                            ...
                            <span className="text-muted-foreground">
                                Total: <span className="text-foreground font-medium">{mainStock}</span>
                            </span>
                        </div>
                        <Progress 
                            value={percentUsed} 
