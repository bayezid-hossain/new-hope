...
export function OfficerAnalytics({ orgId }: { orgId: string }) {
    const trpc = useTRPC();
    ...
    return (
        <div className="space-y-6">
                ...
                <Table>
                    <TableHeader className="bg-slate-50/50">
                        <TableRow>
                            ...
                            <TableHead className="font-semibold text-right px-6">Mortality</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {filtered?.map((officer) => (
                            <TableRow key={officer.officerId} className="hover:bg-slate-50/50 transition-colors">
                                <TableCell className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[11px] text-slate-500">{officer.email}</span>
                                        </div>
                                    </div>
                                </TableCell>
                                <TableCell>
                                    <Badge variant="outline" className="font-bold text-slate-700 bg-slate-50 border-slate-200">
                                        {officer.farmersCount} Farmers
                                    </Badge>
                                </TableCell>
                                <TableCell>
                                    <div className="flex flex-col">
                                        <div className="flex items-center gap-1.5 text-slate-700 font-semibold">
                                            ...
                                            {officer.activeCycles} Active
                                        </div>
                                        <div className="text-[10px] text-slate-400 ml-5">
                                            {officer.pastCycles} Completed
                                        </div>
                                    </div>
                                </TableCell>
                                <TableCell>
                                    <div className="flex items-center gap-1.5 font-bold text-slate-900">
                                        <Bird className="h-4 w-4 text-slate-400" />
                                ...
                                <TableCell className="text-right px-6">
                                    <div className="flex flex-col items-end">
                                        <div className="flex items-center gap-1.5 font-bold text-red-600">
                                            ...
                                            {officer.totalMortality}
                                        </div>
                                        <div className="text-[10px] text-slate-400">
                                            {officer.totalDoc > 0 ? ((officer.totalMortality / officer.totalDoc) * 100).toFixed(1) : 0}% rate
                                        </div>
                                    </div>
                                </TableCell>
                            </TableRow>
            ...
            <div className="md:hidden space-y-4">
                {filtered?.map((officer) => (
                    <Card key={officer.officerId} className="border-slate-200 shadow-sm overflow-hidden bg-white">
                        <CardHeader className="bg-slate-50/50 py-3 flex flex-row items-center justify-between border-b">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-[10px] text-slate-500">{officer.role}</span>
                                </div>
                            </div>
