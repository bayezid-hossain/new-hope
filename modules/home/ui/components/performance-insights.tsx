        ...
        <Card className="col-span-1 md:col-span-2 lg:col-span-4 border-emerald-100 bg-gradient-to-br from-white to-emerald-50/20">
            <CardHeader>
                <CardTitle className="flex items-center gap-2 text-sm xs:text-base">
                    <Trophy className="h-4 w-4 xs:h-5 xs:w-5 text-emerald-500" />
                    Top Performers
                </CardTitle>
                <CardDescription className="text-[10px] xs:text-xs">
                    Best efficiency by mortality rate
                </CardDescription>
            </CardHeader>
            <CardContent>
                            ...
                            return (
                                <div key={farmer.farmerId} className="flex items-center justify-between p-3 bg-white rounded-lg border border-slate-100 shadow-sm">
                                    <div className="flex items-center gap-2 xs:gap-3">
                                        <div className={`flex items-center justify-center h-6 w-6 xs:h-8 xs:w-8 rounded-full font-bold text-[10px] xs:text-xs
                                        ...
                                        <div>
                                            <p className="font-semibold text-xs xs:text-sm text-slate-900">{farmer.farmerName}</p>
                                            <p className="text-[10px] xs:text-xs text-muted-foreground">{farmer.activeCyclesCount} {cycleLabel}{farmer.activeCyclesCount !== 1 ? 's' : ''}</p>
                                        </div>
                                    </div>

                                    <div className="text-right">
                                        <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200 text-[10px] xs:text-xs px-1 xs:px-2">
                                            {farmer.avgMortalityRate.toFixed(1)}%
                                        </Badge>
                                    </div>
                                </div>
                            )
